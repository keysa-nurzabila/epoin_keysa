...
<body>
    ...
    <form action="{{ route( 'akun.update', $akun->id )}}" method="POST" enctype="multype/form-data">
        @csrf
        @method('PUT')
        <h2>Data Akun</h2>
        ...
        <select name="usertype" required>
            <option {{ 'admin' == $akun->usertpe ? 'selected' : '' }} value="admin">Admin</option>
            <option {{ 'ptk' == $akun->usertpe ? 'selected' : '' }} value="ptk">PTK</option>
        </select>
        <br><br>
